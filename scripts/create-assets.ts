...
const arch = (await arg('Enter the architecture')) as Arch;
const release_id = await arg("Enter the release's id");
const tag_name = `v${version}`;

const archs: Arch[] = arch === 'both' ? ['arm64', 'x64'] : [arch];
...
for (const arch of archs) {
  ...
  console.log(`Tar ${newKitPath()} to ${kitTarPath}`);
