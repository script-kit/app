...
export const cacheChoices = (scriptPath: string, choices: Choice[]) => {
	log.info(
		`🎁 Caching choices for ${scriptPath}: Choices ${choices?.length}. First choice name: ${choices[0]?.name}`,
	);
...
export const createMessageMap = (processInfo: ProcessAndPrompt) => {
	...
	const kitMessageMap: ChannelHandler = {
			...
			async (
				...
				widgetState.widgets.push({
					...
					ignoreMeasure: Boolean(
						value?.options?.width || value?.options?.height,
					),
				});
		...
		GET_SCREENS_INFO: onChildChannelOverride(
			async ({ child }, { channel, value }) => {
				...
				const displaysWithThumbnails = await Promise.all(
					displays.map(async (display) => {
						...
						try {
							...
							const displaySource = await getSourceFromRectangle(
								id.toString(),
								bounds,
							);
							if (displaySource) {
								const image = displaySource.thumbnail.toPNG();
								const thumbnailPath = osTmpPath(
									`display-thumbnail-${id}-${randomUUID()}.png`,
								);
		...
		DEBUG_SCRIPT: onChildChannelOverride(async (processInfo, data) => {
			...
			if (processInfo?.child?.pid) {
				processes.removeByPid(
					processInfo?.child?.pid,
					"messages child process cleanup",
				);
			}
		...
		SET_INPUT: onChildChannel(
			({ child }, { channel, value, promptId }, samePrompt) => {
				if (samePrompt) {
		...
		SET_SHORTCUTS: onChildChannel(
			({ child, prompt }, { channel, value, promptId }, samePrompt) => {
				if (samePrompt) {
		...
		SET_PROMPT_DATA: onChildChannel(
			async ({ pap, prompt }, { channel, value, promptId }) => {
				...
				if (samePrompt && value?.choices) {
					...
					const maybeMarkAsUntrusted = (script: Script | Scriptlet) => {
						if (
							script?.kenv &&
							!(
								kitState.trustedKenvs.includes(script.kenv) ||
								defaultTrustedKenvs.includes(script.kenv)
							)
						) {
							(script as any).untrusted = true;
							log.info(
								`Marking ${script.filePath} from kenv:${script.kenv} as untrusted`,
								script,
							);
						}
					};
		...
		SET_TEMP_THEME: onChildChannel(({ child }, { channel, value }) => {
			...
			const appearance = getAppearance(
				parseTheme(kitState.tempTheme || kitState.theme),
			);
		...
		CLIPBOARD_WRITE_BUFFER: onChildChannel(
			async ({ child }, { channel, value }) => {
		...
		REGISTER_GLOBAL_SHORTCUT: onChildChannelOverride(
			({ child, scriptPath }, { channel, value }) => {
		...
		UNREGISTER_GLOBAL_SHORTCUT: onChildChannel(
			({ scriptPath, child }, { channel, value }) => {
			...
			debounce(
				async ({ child }, { channel, value }) => {
					...
					setTimeout(() => {
						...
						childSend({ channel, value });
						log.info(
							`SET SELECTED TEXT DONE with ${channel}`,
							text?.slice(0, 3) + "...",
						);
						setTimeout(() => {
							log.info(
								`RESTORING CLIPBOARD with ${channel}`,
								prevText?.slice(0, 3) + "...",
							);
		...
		ERROR: onChildChannelOverride(({ child }, { channel, value }) => {
			...
			if (errorMap.get(stack) > 2) {
				child.kill("SIGKILL");
				log.info(
					`Killed child ${child.pid} after ${errorMap.get(stack)} of the same stack errors`,
					value,
				);
		...
		STAMP_SCRIPT: onChildChannelOverride(
			async (processInfo: ProcessAndPrompt, { channel, value }) => {
				if (!processInfo.launchedFromMain) {
					log.info(
						`${processInfo.pid}: 🚫 Not stamping pid because it wasn't launched from main`,
					);
					return;
				}
				const stamp: Stamp = {
			...
			childSend({
				...
				value: {
					...
					summary: {
						...
						bound: status.filter((p) => p.boundToProcess && !p.isDestroyed)
							.length,
						orphaned: status.filter(
							(p) => !p.boundToProcess && !p.isIdle && !p.isDestroyed,
						).length,
			...
			childSend({
				...
				value: {
					...
					summary: {
						...
						visiblePrompts: status.filter((p) => p.isVisible && !p.isDestroyed)
							.length,
						boundPrompts: status.filter(
							(p) => p.boundToProcess && !p.isDestroyed,
						).length,
						orphanedPrompts: status.filter(
							(p) => !p.boundToProcess && !p.isIdle && !p.isDestroyed,
						).length,
		...
		FORCE_PROMPT_CLEANUP: onChildChannelOverride(
			({ child }, { channel, value }) => {
