...
import {
  checkProcessAlive as monitorCheck,
  listenForProcessExit as monitorListen,
  startProcessMonitoring as monitorStart,
  stopProcessMonitoring as monitorStop,
} from './prompt.process-monitor';
import { getProcessCheckInterval, shouldMonitorProcess } from './prompt.process-utils';
...
export const startProcessMonitoringImpl = (prompt: any): void => {
  ...
  if (
    !shouldMonitorProcess({
      scriptPath: prompt.scriptPath,
      scriptName: prompt.scriptName,
      kenvEnv: kitState?.kenvEnv as any,
    })
  ) {
