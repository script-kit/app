...
export const setTheme = (value: string, reason = "") => {
	...
	kitState.themeName =
		platformSpecificTheme.match(/--name:\s*"([^"]+)"/)?.[1] || "";
...
const createChild = ({
	...
	const kitLoadingHandler = (data) => {
		if (
			data?.channel === Channel.KIT_LOADING ||
			data?.channel === Channel.KIT_READY
		) {
			processLog.info(
...
class Processes extends Array<ProcessAndPrompt> {
	...
	public add(
		...
		const prompt = prompts.attachIdlePromptToProcess(
			"➕ add process",
			child.pid,
		);
	...
	public removeAllRunningProcesses() {
		...
		for (const { pid, scriptPath } of runningIds) {
			processLog.info(
				`🔥 Attempt removeAllRunningProcesses: ${pid} - ${scriptPath}`,
			);
...
setInterval(() => {
	try {
		...
		const visiblePrompts = promptStatus.filter(
			(p) => p.isVisible && !p.isDestroyed,
		);
		const boundPrompts = promptStatus.filter(
			(p) => p.boundToProcess && !p.isDestroyed,
		);
		const orphanedPrompts = promptStatus.filter(
