...
// Import after mocks
import { startServer, stopServer, expressApp } from './server';
...
describe('HTTP Server', () => {
  ...
  beforeEach(() => {
    ...
    mockApp = {
      ...
      listen: vi.fn().mockImplementation((port, cb) => {
        process.nextTick(cb);
        return mockServer;
      })
    };
  ...
  describe('startServer', () => {
    ...
    it('should start HTTPS server when certificates exist', async () => {
      ...
      const mockHttpsServer = {
        listen: vi.fn().mockImplementation((p, cb) => {
          process.nextTick(cb);
  ...
  describe('Route handlers', () => {
    ...
    it('should handle nested folder paths', async () => {
      const nestedHandler = mockApp.get.mock.calls.find((call: any) => 
        call[0] === '/:folder/:subfolder/:script'
      )[1];
  ...
  describe('CORS configuration', () => {
    it('should allow all origins', async () => {
      ...
      const corsMiddleware = mockApp.use.mock.calls.find((call: any) => 
        call[0].toString().includes('cors')
      );
