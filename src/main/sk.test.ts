...
describe('Unix Socket Server (sk.ts)', () => {
  ...
  beforeEach(() => {
    ...
    mockServer.listen = vi.fn().mockImplementation((path, cb) => {
      process.nextTick(cb);
      return mockServer;
    });
  ...
  describe('initKit', () => {
    ...
    it('should handle socket file cleanup on error', async () => {
      ...
      mockServer.listen.mockImplementation((path, cb) => {
        process.nextTick(() => {
  ...
  describe('Socket communication', () => {
    ...
    it('should handle GET-style request', async () => {
      ...
      expect(mockSocket.write).toHaveBeenCalledWith(
        expect.stringContaining('HTTP/1.0 200 OK')
      );
      expect(mockSocket.write).toHaveBeenCalledWith(
        expect.stringContaining(JSON.stringify({ status: 'ok' }))
      );
    });
    ...
    it('should handle script errors', async () => {
      ...
      expect(mockSocket.write).toHaveBeenCalledWith(
        JSON.stringify({ error: 'Script failed' })
      );
      expect(mockSocket.end).toHaveBeenCalled();
    });
    ...
    it('should parse query string arguments correctly', async () => {
      ...
      mockSocket.emit('data', Buffer.from(
        'GET /utils/encode?text=hello%20world&encode=true HTTP/1.0\r\n\r\n'
      ));
