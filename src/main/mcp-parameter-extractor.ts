 ...
 */
export async function extractMCPToolParameters(
  code: string,
): Promise<MCPToolParameter[] | { toolConfig: any } | { inputSchema: any }> {
  ...
  function walk(node: any, parent: any = null) {
    ...
    if (node.type === 'AwaitExpression' && node.argument?.type === 'TSAsExpression') {
      ...
      if (tsAsExpr.expression?.type === 'CallExpression' && tsAsExpr.expression.callee?.name === 'tool') {
    ...
    if (!foundToolCall && !foundParamsCall && node.type === 'CallExpression' && node.callee?.name === 'arg') {
      ...
      if (objectArg) {
        const placeholderProp = objectArg.properties.find(
          (p: any) => p.key && (p.key.name === 'placeholder' || p.key.value === 'placeholder'),
        );
