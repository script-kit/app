...
import { type IDisposable, type editor as monacoEditor, Range } from 'monaco-editor/esm/vs/editor/editor.api';
import { useCallback, useEffect, useRef, useState } from 'react';
import { initializeMonacoEditor } from '../utils/monaco';

const { ipcRenderer } = window.electron;

import { isReservedEditorShortcut, toMonacoKeybindingOrUndefined } from '@renderer/utils/keycodes';
...
export default function Editor() {
  ...
  useEffect(() => {
    ...
    const replaceTextRange = (_event: any, { start, end, text }: { start: number; end: number; text: string }) => {
    ...
    const getLineInfo = (_event: any, lineNumber?: number) => {
      if (!editor || !editor.getModel()) return;
    ...
    const findAndReplaceAll = (
      _event: any,
      {
        ...
        searchText: string;
        replaceText: string;
        options?: { regex?: boolean; matchCase?: boolean; wholeWord?: boolean };
      },
    ) => {
      if (!editor || !editor.getModel()) return;
    ...
    const setFoldedRegions = (_event: any, regions: Array<{ start: number; end: number }>) => {
      if (!editor) return;
      ...
      regions.forEach((region) => {
        editor.setSelection(new Range(region.start, 1, region.end, 1));
    ...
    const executeMonacoCommand = async (_event: any, { commandId, args }: { commandId: string; args?: any }) => {
      if (!editor) return;
    ...
    const scrollToPosition = (_event: any, position: 'top' | 'center' | 'bottom' | number) => {
      if (!editor) return;
